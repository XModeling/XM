...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  2 16:12:31 2020

@author: Federico Bianchi, Francesco Masillo, Alberto Castellini, Alessandro Farinelli
"""
...
def weather_files():
    ...
    for idx in range(len(data)-horizon):
        ...
        temp.to_csv('Runtime/Weather_forecast/forecast_from_%s.csv' %data.loc[idx, 'day'])
...
def forecast_module():
    ...
    if(mode == 'demo'):
        ...
        df.to_csv('Forecast_files/forecast_from_%s.csv' %(current_date))
    else:
        results.to_csv('Forecast_files/forecast_from_%s.csv' %(current_date))
...
def create_hourly_model(X_dict, y_dict,current_date):
    ...
    os.mkdir('Model/model_of_%s' %current_date)
    ...
    for day in range(7):
        for hour in range(24):
            ...
            if(x1.empty != True):
                ...
                model.save('Model/model_of_%s/day_%i_hour_%i.pickle' %(current_date, day, hour))
...
def load_model(current_date):
    ...
    for cols in range(7):
        for rows in range(24):
            models.loc[rows, cols] = sm.load('Model/model_of_%s/day_%i_hour_%i.pickle' %(current_date,cols,rows))               
...
def parameters_demo(model, current_dt):
    ...
    print(current_dt)
    os.mkdir('Performance/Parameters/from_%s' %(current_dt))
    
    os.mkdir('Performance/Parameters/from_%s/full_representation' %(current_dt))
    
    for x in features[1:]:
        avg_dictionary[x].to_csv('Performance/Parameters/from_%s/%s_from_%s.csv' %(current_dt, x, current_dt))
        os.mkdir('Performance/Parameters/from_%s/full_representation/%s' %(current_dt, x))
        dictionary[x].to_csv('Performance/Parameters/from_%s/full_representation/%s/%s_from_%s.csv' %(current_dt, x, x, current_dt))
...
def rsquared(model, current_dt):
    ...
    os.mkdir('Performance/Rsquared/%s' %current_dt)
    
    df.to_csv('Performance/Rsquared/%s/rsquared.csv' %current_dt)


# Returns the last weather forecasting
def weather_forecast(dt):    
   
   df = pd.read_csv('Runtime/Weather_forecast/forecast_from_%s.csv' %dt, index_col=0)
...
def set_holidays(data):
    ...
    cal_18 = Italy().holidays(2018)
            
    data['h'] = pd.Series(0, index=data.index)
    
    for hol in cal_18:
        data.loc[data['day'].dt.date == hol[0], 'h'] = 1
...
def forecasting(model, data):
    ...
    if (horizon <= 24):
        ...
        for idx in range(168, len(temp_data)):
            ...
            results.loc[idx, 'pred'] = model.loc[temp_data.loc[idx, 'day'].hour][temp_data.loc[idx, 'day'].weekday()].predict(temp_data.loc[idx, 't':].values)[0]
        ...
        for idx in range(168):
            ...
            if(iteration + 1 <= 24):
                ...
                results.loc[idx, 'pred'] = (model.loc[temp_data.loc[idx + 168, 'day'].hour][temp_data.loc[idx + 168, 'day'].weekday()].predict(temp_data.loc[idx + 168, 't':].values))[0] 
                iteration += 1
...
def metrics_computation_demo(current_dt):
    ...
    os.mkdir('Performance/Demo/%s' %current_dt)
    rmse_demo.to_csv('Performance/Demo/%s/rmse_demo.csv' %current_dt)
...
class mainWindow:
    def __init__(self, top=None, pathDir=None):
        ...
        self.ButtonStart.place(relx=0.88, rely=0.008, height=25, width=50)
        ...
        self.addClickable(self.figLastEpoch)
        #self.drawLastEpoch()
        ...
        self.optionsVarChoice = ["All","h","l1","l2","l3","l4","l5","l6","l7","lp","r","rh","t","t2","tm","tm2","tmv","tmy","tmy2","wd","ws"]
    ...
    def changeSele48H(self, force=True):
        ...
        if self.var48H.get() != "None":
            ...
            dataset = pd.read_csv(self.pathDir+"/Runtime/Weather_forecast/forecast_from_"+str(self.now)+".csv")#.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def draw48H(self):
        ...
        dataset = pd.read_csv(pathToUse+"forecast_from_"+str(self.now)+".csv")#.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def drawLastEpoch(self):
        ...
        for fi in files:
            ...
            dataset = pd.read_csv(self.pathDir+"/Performance/Rsquared/"+pastString+" 00:00:00/rsquared.csv", index_col=0)
            self.r2.append(np.mean(dataset.values.ravel()))
        if len(self.rmse) > 720:
            self.rmse = self.rmse[len(self.rmse)-720:]
            self.r2 = self.r2[len(self.r2)-720:]
        if len(self.rmse) == 1:
            self.axLastEpoch.scatter(self.days, self.rmse, label="RMSE")
            self.axLastEpochTwin.scatter(self.days, self.r2, c="orange", label="R^2")
        else:
    ...
    def updateLastEpoch(self):
        ...
        self.rmse.append(math.sqrt(mean_squared_error(dataset["l"], dataset["pred"])))
        self.days.append(dataset["day"].iloc[0])
        dataset = pd.read_csv(self.pathDir+"/Performance/Rsquared/"+str(self.now).split(" ")[0]+" 00:00:00/rsquared.csv", index_col=0)#.strftime("%Y-%m-%d")
        self.r2.append(np.mean(dataset.values.ravel()))
        if len(self.rmse) > 720:
    ...
    def getChoice(self):
        ...
        if flagDayAll == True and flagHourAll == True and flagVarAll==True:
            for var in varToUse:
                pathToUse = self.pathDir+"/Performance/Parameters/from_"+str(self.now).split(" ")[0]+" 00:00:00/full_representation/"+var+"/"+var+"_from_"+str(self.now).split(" ")[0]+" 00:00:00.csv"
            ...
            pathToUse = self.pathDir+"/Performance/Parameters/from_"+str(self.now).split(" ")[0]+" 00:00:00/full_representation/"+varToUse+"/"+varToUse+"_from_"+self.now.strftime("%Y-%m-%d")+" 00:00:00.csv"
            dataset = pd.read_csv(pathToUse, index_col = 0)
            result = dataset.values
        elif flagDayAll == True and flagHourAll == False and flagVarAll==True:
            for var in varToUse:
                pathToUse = self.pathDir+"/Performance/Parameters/from_"+str(self.now).split(" ")[0]+" 00:00:00/full_representation/"+var+"/"+var+"_from_"+self.now.strftime("%Y-%m-%d")+" 00:00:00.csv"
                dataset = pd.read_csv(pathToUse, index_col = 0)
            ...
            pathToUse = self.pathDir+"/Performance/Parameters/from_"+str(self.now).split(" ")[0]+" 00:00:00/full_representation/"+varToUse+"/"+varToUse+"_from_"+str(self.now).split(" ")[0]+" 00:00:00.csv"
            dataset = pd.read_csv(pathToUse, index_col = 0)
    ...
    def addClickable(self, figure):
        def on_click(event):
            ...
            ax = fig2.axes[0]
            ax.set_title(str(ax.get_title())+", "+str(self.now))
            ...
            tool.update()
            self.root.update()
            #_thread.start_new_thread(self.duplicateFig, (figure,))
