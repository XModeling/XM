...
class TICC:
    def __init__(self, window_size=10, number_of_clusters=5, lambda_parameter=11e-2,
                 beta=400, maxIters=100, threshold=2e-5, write_out_file=False,
                 prefix_string="", num_proc=1, compute_BIC=True, cluster_reassignment=20, seed=102):
        """
        Parameters:
    ...
    def fit(self, input_file):
        ...
        if self.compute_BIC:
            bic = computeBIC(self.number_of_clusters, time_series_rows_size, clustered_points, train_cluster_inverse,
                             empirical_covariances)
            aic = computeAIC(self.number_of_clusters, time_series_rows_size, clustered_points, train_cluster_inverse,
                             empirical_covariances)
            ll = computeLL(self.number_of_clusters, time_series_rows_size, clustered_points, train_cluster_inverse,
                             empirical_covariances)
            return clustered_points, train_cluster_inverse, bic, aic, ll
    ...
    def load_data(self, input_file):
        ...
        Data = Data.drop(["latitude","longitude","experiment"], axis=1)
        print(Data)
        Data = np.array(Data)
