...
def general_info(righe=0, colonne=0, nclust=0, sse=0, percorsoD="/", percorsoS="/", accuracy_local="Nan", ce_local="Nan", f1_local="Nan", entropy_local="Nan", purity_local='Nan', nb_clusters=0):
    gi = pd.DataFrame()
    df, latitude, longitude, experiment = get_dataset(percorsoD)
    varU = list(df)
    ...
    gi["Values"] = [percorsoD, percorsoS, colonne, righe, "K-Means", nclust, accuracy_local, ce_local, f1_local, entropy_local, purity_local, nb_clusters,"-"]
...
def kmeans(numerocl, percorsoD="", percorsoS="", percorsoG="", percorsoDN=""):

    percorsoS = percorsoS+"/K-Means"
    if not os.path.exists(percorsoS):
        os.makedirs(percorsoS)
    df, latitude, longitude, experiments = get_dataset(percorsoD)
    km = KMeans(n_clusters=int(numerocl), n_jobs=-1, n_init=300)
    print "[XM]> ====== Creating K-Means Clustering Model ======"
    km = km.fit(df)
    results = km.predict(df)
    ...
    if percorsoG != "":
        ...
        df_evaluation["clusters_found"] = list(km.predict(df))
        ...
        general_info(righe=df.shape[0], colonne=df.shape[1], nclust=numerocl, sse=sse, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local=evaluation_temp[0], ce_local=evaluation_temp[1], f1_local=evaluation_temp[2], entropy_local=evaluation_temp[3], purity_local=evaluation_temp[4], nb_clusters=pd.unique(results).size)
        if percorsoDN != "":
        ...
        general_info(righe=df.shape[0], colonne=df.shape[1], nclust=numerocl, sse=sse, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local="NA", ce_local="NA", f1_local="NA", entropy_local="NA", purity_local="NA", nb_clusters=pd.unique(results).size)
        if percorsoDN != "":
            fileDN = open(percorsoDN, 'r')
            fileDND = open(percorsoS+"/dataStandardization.csv", 'w')
            for line in fileDN:
