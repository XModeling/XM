...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 29 14:46:40 2018
...
def general_info(righe=0, colonne=0, nclust=0, cov="", ll=0, percorsoD="/", percorsoS="/", accuracy_local="Nan", ce_local="Nan", f1_local="Nan", entropy_local="Nan", purity_local="Nan", nb_clusters=0):
    gi = pd.DataFrame()
    df, latitude, longitude, experiment = get_dataset(percorsoD)
    varU = list(df)
    gi["Parameter"] = [ "Dataset Path","Results Path","Number of columns","Number of rows","Method","N. Cluster","Covariance","Accuracy","CE","F1","Entropy","Purity","NbClusterFound","Note"]
    gi["Values"] = [percorsoD, percorsoS, colonne, righe, "GMM", nclust, cov, accuracy_local, ce_local, f1_local, entropy_local, purity_local, nb_clusters,"-"]
...
def gmm(ncomp, cov, re_init, percorsoD="", percorsoS="", percorsoG="", percorsoDN=""):
    percorsoS = percorsoS+"/GMM"
    if not os.path.exists(percorsoS):
        os.makedirs(percorsoS)
    df, latitude, longitude, experiments = get_dataset(percorsoD)
    gm = GMM(n_components=int(ncomp), covariance_type=cov, n_init=int(re_init), verbose=1, verbose_interval=50)
    print "[XM]> ====== Creating GMM Clustering Model ======"
    gm = gm.fit(df)
    results = gm.predict(df)
    ...
    means = gm.means_
    means = pd.DataFrame(means).to_string()
    centri = open(percorsoS+"/model_parameters.txt","w")
    centri.write("{}".format(means))
    centri.close()
    if cov == 'full':
        cova = gm.covariances_
        indexes = np.triu_indices(len(list(df)),1)
        matr = []
        matr_std = []
        for covi in cova:
            matr.append(covi[indexes])
            matr_std.append(np.diag(covi))
        matr = pd.DataFrame(matr).to_string()
        matr_std = pd.DataFrame(matr_std).to_string()
        centri = open(percorsoS+"/model_parameters_cov.txt","w")
        centri.write("{}".format(matr))
        centri.close()
        centri = open(percorsoS+"/model_parameters_std.txt","w")
        centri.write("{}".format(matr_std))
        centri.close()
    else:
        cova = gm.covariances_
        cova = pd.DataFrame(cova).to_string()
        centri = open(percorsoS+"/model_parameters_std.txt","w")
        centri.write("{}".format(cova))
        centri.close()
    ...
    if percorsoG != "":
        ...
        df_evaluation["clusters_found"] = list(gm.predict(df))
        ...
        general_info(righe=df.shape[0], colonne=df.shape[1], nclust=ncomp, cov=cov, ll=ll, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local=evaluation_temp[0], ce_local=evaluation_temp[1], f1_local=evaluation_temp[2], entropy_local=evaluation_temp[3], purity_local=evaluation_temp[4], nb_clusters=pd.unique(results).size)
        if percorsoDN != "":
        ...
        general_info(righe=df.shape[0], colonne=df.shape[1], nclust=ncomp, cov=cov, ll=ll, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local="NA", ce_local="NA", f1_local="NA", entropy_local="NA", purity_local="NA", nb_clusters=pd.unique(results).size)
        if percorsoDN != "":
            fileDN = open(percorsoDN, 'r')
            fileDND = open(percorsoS+"/dataStandardization.csv", 'w')
            for line in fileDN:
