...
def general_info(righe=0, colonne=0, clust=0, window=0, p_lambda=0, beta=0, percorsoD="/", percorsoS="/", accuracy_local="Nan", ce_local="Nan", f1_local="Nan", entropy_local="Nan", purity_local="Nan", nb_clusters_found=0):
...
def genera_cluster(cluster=0, window=0, p_lambda=0, beta=0, percorsoD="/", percorsoS="/", percorsoG="", percorsoDN="", seed=102):
    ...
    (cluster_assignment, cluster_MRFs) = ticc.fit(input_file=percorsoD)
    ...
    if percorsoG != "":
        ...
        general_info(righe=df.shape[0], colonne=df.shape[1], clust=cluster, window=window, p_lambda=p_lambda, beta=beta, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local=evaluation_temp[0], ce_local=evaluation_temp[1], f1_local=evaluation_temp[2], entropy_local=evaluation_temp[3], purity_local=evaluation_temp[4], nb_clusters_found=evaluation_temp[5])
        if percorsoDN != "":
        ...
        for cl, mat in enumerate(coordinate):
            ...
            centri.close()
        print "[XM]> Model parameters generated"
        general_info(righe=df.shape[0], colonne=df.shape[1], clust=cluster, window=window, p_lambda=p_lambda, beta=beta, percorsoD=percorsoD, percorsoS=percorsoS, accuracy_local="NA", ce_local="NA", f1_local="NA", entropy_local="NA", purity_local="NA", nb_clusters_found=np.unique(clusters_found).size)
        if percorsoDN != "":
            fileDN = open(percorsoDN, 'r')
            fileDND = open(percorsoS+"/dataStandardization.csv", 'w')
            for line in fileDN:
