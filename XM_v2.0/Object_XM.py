...
def handler():
    ...
    if (var.get()=="All") and (varG.get()=="Geo-localization"):
        ...
        if len(latitude) != 0:
            ...
            tkMessageBox.showwarning("Invalid choice", "The dataset does not contain latitude-longitude data")
        return
    if (var.get()!="All") and (varG.get()=="Geo-localization"): 
        df, latitude, longitude, experiment = graph.get_dataset(percorsoD, percorsoS+"/", False)
        if len(latitude) != 0:
    ...
    if (var.get()=="All") and (varG.get()=="Dimensionality Reduction"):
        try:
            ...
            tkMessageBox.showwarning("Invalid choice", "The result folder does not contain tsne.csv (tsne data)")
        return
    if (var.get()!="All") and (varG.get()=="Dimensionality Reduction"):
        try:
            tsne = open(percorsoS+"/tsne.csv", 'r')
...
def first(nc, means, finestra, list_slaves, sidebar):
    ...
    if obj.pathS != "/":
        ...
        righe, colonne = obj.dataset.shape
        ...
        try:
            fileDN = open(obj.pathS+"dataStandardization.csv", 'r')
...
def draw_graph(finestra, list_slaves, percorsoD, percorsoS, percorsoG="", norm=False, exp="All"):    
    #try:
        ...
        my_canvas = Canvas(contC, background='white')
        my_canvas.pack(side=BOTTOM, fill=BOTH, expand=1)
        figura = obj.draw_timeseries(norm, cb.state(), exp)
        canvas = FigureCanvasTkAgg(figura, my_canvas)
        canvas.get_tk_widget().pack(fill=BOTH, expand=1)
        #canvas.draw()
        tool = NavigationToolbar2TkAgg(canvas, contC)
...
def create_general_info(finestra, list_slaves, norm=False):  
    global gt
    gt = False
    #try:
    ls = Splash(finestra, "IMG/hg.gif")
...
def on_load(finestra, list_slaves, percorsoD, percorsoS, percorsoG="", norm=False):
    ...
    if percorsoS != "/":
        ...
        sidebar.pack(side=TOP, fill=X)
        global obj
        clust = XM_clustering_results(percorsoD, percorsoS, percorsoG)
        obj = clust
        do_nc(percorsoS)
        first(nc, means, finestra, list_slaves, finestra.pack_slaves()[1])
        create_general_info(finestra, list_slaves, norm)
        ls.__exit__(None, None, None)
    else:
...
def model_h(finestra, list_slaves, percorsoD, percorsoS, percorsoG="", norm=False, numero=0, exp="All"):
    ...
    if figura == 0:
        ls.__exit__(None,None,None)
        tkMessageBox.showwarning("Invalid choice", "No points correspond to selected model + experiment")
        return
    canvas = FigureCanvasTkAgg(figura, my_canvas)
    canvas.get_tk_widget().pack(fill=BOTH, expand=1)
    #canvas.draw()
...
def create_images(finestra, percorsoD, percorsoS, percorsoG, norm, usedV, numero=0):
    ...
    tkMessageBox.showinfo("End of generation", "You can find the images in "+directory)
