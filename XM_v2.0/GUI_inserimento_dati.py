...
def comp_Default(finestra, root, status, entry, percorsoD, percorsoS, percorsoG, percorsoDN):
    ...
    print(percorsoS)
    if entry == "":
        tkMessageBox.showwarning("WARNING", "Specify number of clusters")
...
def comp_Custom(finestra, root, status, entry1, entry2, entry3, percorsoD, percorsoS, percorsoG, percorsoDN):
    ...
    status.update()
    if entry1 == "":
        tkMessageBox.showwarning("WARNING", "Specify SDMax")
        status.configure(text="Ready")
        return
    if entry2 == "":
        tkMessageBox.showwarning("WARNING", "Specify N")
        status.configure(text="Ready")
        return
    if entry3 == "":
        tkMessageBox.showwarning("WARNING", "Specify NbIter")
        status.configure(text="Ready")
        return
...
def compute_tsne(finestra, root, status, percorsoD, percorsoS, samples):
    ...
    status.update()
    if samples == "":
        tkMessageBox.showwarning("WARNING", "Specify number of samples")
        status.configure(text="Ready")
        return
    sc.tsne(percorsoD, percorsoS, samples)
    status.configure(text="Ready")
    tkMessageBox.showinfo("Finish", "The t-SNE process has ended successfully")
    root.destroy()
    XM.build_up()
...
def compute_kmeans(finestra, root, status, percorsoD, percorsoS, percorsoG, percorsoDN, clusters):
    ...
    status.configure(text="Ready")
    tkMessageBox.showinfo("Finish", "The KMEANS process has ended successfully")
    root.destroy()
    XM.build_up()
...
def compute_gmm(finestra, root, status, percorsoD, percorsoS, percorsoG, percorsoDN, clusters, iniz, cov):
    ...
    if clusters == "":
        ...
        return
    if iniz == "":
        tkMessageBox.showwarning("WARNING", "Specify number of re-initialization")
        status.configure(text="Ready")
        return
    if cov == "":
        tkMessageBox.showwarning("WARNING", "Select a type of variance")
        status.configure(text="Ready")
        return
    if cov == "D":
        cov = 'diag'
    ...
    status.configure(text="Ready")
    tkMessageBox.showinfo("Finish", "The GMM process has ended successfully")
    root.destroy()
    XM.build_up()
...
def compute_spectral(finestra, root, status, percorsoD, percorsoS, percorsoG, percorsoDN, clusters):
    ...
    status.configure(text="Ready")
    tkMessageBox.showinfo("Finish", "The Spectral clustering process has ended successfully")
    root.destroy()
    XM.build_up()
...
def compute_ticc(finestra, root, status, percorsoD, percorsoS, percorsoG, percorsoDN, clusters, window, p_lambda, beta):
    ...
    status.configure(text="Ready")
    tkMessageBox.showinfo("Finish", "The TICC process has ended successfully")
    root.destroy()
    XM.build_up()
